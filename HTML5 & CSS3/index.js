document.addEventListener("DOMContentLoaded", function() {
    ...
    for (let i = 0; i < totalCards; i++) {
        ...
        const imageNumber = (i % imageCount) + 2; // Start from hik2 to hik27
        const imageName = `hik${imageNumber}.jpg`;
        const productTitle = cameraNames[i % cameraNames.length]; // Get camera name from array
    ...
    productGrid.addEventListener("click", function(e) {
        if (e.target.classList.contains("product-card")) {
            ...
            const price = e.target.querySelector("p").textContent;
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function createProductCard(index) {
            ...
            const imageName = `hik${imageNumber}.jpg`;
            const productTitle = `Product ${index + 1}`;
            const productPrice = `$${(Math.random() * 100 + 10).toFixed(2)}`;
